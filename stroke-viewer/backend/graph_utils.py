...
def make_anchor_onehots(C_init):
    """
    Automatically pick one anchor per component based on max assignment.
    For each component k, the row with the largest C_init[i, k] becomes the anchor.

    Returns:
      C0      : copy of C_init with anchors forced to one-hot
      mask    : (N,) bool array, True for anchor rows
      onehots : (N x K) array of one-hot rows (zero elsewhere)
      anchors : list of row indices chosen as anchors per component
    """
    ...
    for k in range(K):
        # pick the row with the maximum value for component k
        i = np.argmax(C_init[:, k])
        anchors.append(i)
        mask[i] = True
        onehots[i, k] = 1.0
        C0[i, :] = 0.0
...
def pick_anchors(C_init):
    """
    Args:
        C_init: array-like of shape (num_strokes, num_components)
    Returns:
        anchors: list of length num_components
                 anchors[k] = row index of the stroke chosen for component k
    """
    C_init = np.asarray(C_init)
    # For each column (component), take the row index of the max value
    return list(np.argmax(C_init, axis=0))
